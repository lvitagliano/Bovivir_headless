...
{ !menuWP.isLoading && tabs.unshift(menuTienda) && tabs.unshift(menuWP) && tabs.unshift(menuBon) && <NavTabs>
          {tabs &&
            tabs.map(tab => (
              <NavTab key={tab.as} href={tab.href} as={tab.as} label={tab.text} prefetch="visible">
                {tab.items && tab.items.length > 0 && (
                  <div style={{ padding: 20 }}>
                    ...
                    >
                      {subcategory.child_items ? <div>{capitalize(subcategory.title.toLowerCase())}<ul>
                      {subcategory.child_items.map(item => (
                        ...
                        className={classes.link}
                      >
                        {item.title ? capitalize(item.title.toLowerCase()) : capitalize(item.post_title.toLowerCase())}
                      </Link>
                      ))}
                    </ul></div>  : subcategory.title ? capitalize(subcategory.title.toLowerCase()) : capitalize(subcategory.post_title.toLowerCase())}
                    </Link>
                    ))}
                  </div>
                )}
