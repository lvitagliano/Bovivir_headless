...
function AppBar({ tabs }) {
  ...
  return (
    <Paper
      square
      elevation={0}
      style={{ marginTop: '65px', width: '100%', position: 'absolute', top: 0 }}
    >
      <NavBar id="menu">
        {
          <ChevronLeftIcon
            ...
            style={{ visibility: `${tabs[indexes.left - 1] ? 'visible' : 'hidden'}` }}
          />
        }
        ...
        </ul>
        {
          <ChevronRightIcon
            ...
            style={{ visibility: `${tabs[indexes.right + 1] ? 'visible' : 'hidden'}` }}
          />
        }
      </NavBar>
    </Paper>
