import { fetchCmsBlocks, normalizeCmsBlocks } from 'm2-simple-connector/cms/blocks'
import { fetchSubcategory, normalizeSubcategory } from 'm2-simple-connector/subcategory'
import { fetchSubcategoryId, normalizeSubcategoryId } from 'm2-simple-connector/subcategory/id'
import {
  fetchSubcategorySubCategories,
  normalizeSubcategorySubCategories,
} from 'm2-simple-connector/subcategory/sub-categories'
...
async function subcategoryHandler(req, res) {
  ...
  const pageData = {
    ...
    totalPages: get(data, 'totalPages', 0) + 1,
    isLanding,
    cmsBlocks,
    ...
    sortOptions: get(data, 'sortOptions', [])
      .map(option => [
        {
          name: `${get(option, 'name')} Descendente`,
          code: `${get(option, 'code')}: DESC`,
        },
        {
          name: `${get(option, 'name')} Ascendente`,
