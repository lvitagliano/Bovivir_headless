...
const Subcategory = lazyProps => {
  ...
  if (store.search !== undefined) {
    pageData.products = store.search
  }
  ...
  return (
    <>
      ...
      <SearchResultsProvider store={store} updateStore={updateStore}>
        <Container maxWidth="lg" style={{ paddingTop: theme.spacing(2), background: '#ededed' }}>
          ...
          <Hbox align="flex-start">
            <Hidden implementation="css" xsDown>
              <div className={classes.sideBar}>
                <Hidden xsDown>
                  ...
                  <Filter classes={{ root: classes.sideBar }} submitOnChange title="Filtrar por" />
                </Hidden>
              </div>
            </Hidden>
            <Grid container style={{ position: 'relative' }}>
              ...
              <Grid item xs={12}>
                {!loading && (
                  <ShowMore className={classes.showmore} variant="button">
                    Mostrar Mas
                  </ShowMore>
                )}
              </Grid>
